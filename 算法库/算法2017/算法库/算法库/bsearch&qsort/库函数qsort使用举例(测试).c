...
int main()
{
	...
	char *d[3] = {"Good morning!", "Good afternoon!", "Good evening!"}; // 定义字符串数组(指针的数组)
	struct TIME e[5] = {{8, 30, 45}, {0, 0, 0}, {23, 59, 59}, {12, 0, 0}, {8, 30, 15}}; // 定义时间数组
	...
	StringArrayOutput(d, 3); // 输出字符数组(排序前)
	qsort(d, 3, sizeof(char*), strcmp); // 快速排序
	StringArrayOutput(d, 3); // 输出字符数组(排序后)
	putchar('\n');

	TimeArrayOutput(e, 5); // 输出时间数组(排序前)
	qsort(e, 5, sizeof(struct TIME), TimeCompare); // 快速排序
	TimeArrayOutput(e, 5); // 输出时间数组(排序后)
	return 0;
}
