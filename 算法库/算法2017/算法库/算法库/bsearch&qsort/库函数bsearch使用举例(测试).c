...
// 输出字符串数组
void StringArrayOutput(const char* const array[], int size)
{
	int k;
	putchar('(');
	for (k = 0; k < size; k++)
...
int main()
{
	...
	char* d[3] = {"Good afternoon!", "Good evening!", "Good morning!"}, *kd = "Good morning!", **pd; // 定义字符串数组(指针的数组), 关键字和行指针
	struct TIME e[5] = {{0, 0, 0}, {8, 30, 15}, {8, 30, 45}, {12, 0, 0}, {23, 59, 59}}, ke = {8, 30, 15}, *pe; // 定义时间数组, 关键字和指针
	...
	StringArrayOutput(d, 3); // 输出字符串数组
	puts(kd); // 输出关键字
	pd = (char**)bsearch(&kd, d, 3, sizeof(char*), StringCompare); // 快速查找
	pd ? printf("[%d]\n", pd - d + 1) : puts("Failed!"); // 输出下标或"Failed"
	putchar('\n');

	TimeArrayOutput(e, 5); // 输出时间数组
	printf("%02d:%02d:%02d\n", ke.hour, ke.minute, ke.second); // 输出关键字
	pe = (struct TIME*)bsearch(&ke, e, 5, sizeof(struct TIME), TimeCompare); // 快速查找
	pe ? printf("[%d]\n", pe - e + 1) : puts("Failed!"); // 输出下标或"Failed"
	return 0;
}
