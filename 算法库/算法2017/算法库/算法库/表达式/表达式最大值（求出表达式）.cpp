...
int used[200];//只输出一个表达式，used[i]为a[i]是否已被输出的标志 
void calc(int i,int j,int k)//输出所有表达式的算法稍微有点麻烦 
{
	if(i>j || used[i]) return ;
	if(k==0)
	{
	   while(i<j) 
	   {
	...
	{   int m,s;
		...
		{
			if(used[i]) break;
			for(s=k-j+m;s<=m-i && s<=k;s++)
			{
				if(s < 0 || k-s-1 < 0) continue;//s为左侧*个数，k-s-1为右侧最少*个数 
			    if(used[i]) break;
				if(k-s>=0 && tag[i][j][k]==tag[i][m][s]+tag[m+1][j][k-s])
				{
					calc(i,m,s);
					cout<<"+";
					calc(m+1,j,k-s);			
				}
				else if(tag[i][j][k]==tag[i][m][s]*tag[m+1][j][k-1-s])
				{
					if(i<m) cout<<"(";
					calc(i,m,s);
					if(i<m) cout<<")";
...
int main()
{
	...
	while (scanf("%d%d", &n, &k) != EOF)
	{
		...
		calc(0,n-1,k);//通过tag[][][]求出能算出最大值的一个具体表达式 
		cout<<endl;
	}
